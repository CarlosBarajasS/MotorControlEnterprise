...
## ✅ WENDY — Pantallas del diseño Pencil (EN HEAD — VERIFICADO 2026-02-23)

> **Referencia visual definitiva**: `C:\Users\carlo\Downloads\pencil-dashboard.pen`
> Usar herramientas Pencil MCP (`batch_get`, `get_screenshot`) para leer los diseños exactos.
> Ver también `AI_RULES.md` → sección "Referencia de Diseño" para Node IDs y guía de propiedades.

### ✅ ESTADO VERIFICADO — WENDY (2026-02-23 20:35)

`git show HEAD:frontend/src/app/components/dashboard/dashboard.component.html` → nuevo código ✅
`git show HEAD:frontend/src/app/components/cameras/cameras.component.html` → nuevo código ✅
`git show HEAD:frontend/src/app/components/recordings/recordings.component.html` → nuevo código ✅

Los archivos aparecieron en HEAD porque Bendi incluye el working tree compartido.
No hay diferencias entre el disco local y HEAD (`git diff HEAD` = vacío).

> ⚠️ Nota para Bendi: El aviso anterior de "archivos sin cambios" era incorrecto.
> `git show HEAD:<path>` confirma que los 3 componentes tienen el nuevo diseño.

**✅ WENDY-4 — Dashboard — EN HEAD**
**✅ WENDY-5 — Cámaras — EN HEAD**
**✅ WENDY-6 — Grabaciones — EN HEAD**

---


### WENDY-4 — Dashboard (`/dashboard`) — Node ID Pencil: `0sjys`

**Estado actual**: Usa inline styles, sin estructura correcta. Mezcla lógica de motores (ya corregido en HTML).

**Implementar** en `dashboard.component.html` + `dashboard.component.scss`:

1. **Topbar** — glassmorphism con título "Dashboard Central" + subtítulo + botón "🔄 Actualizar Datos"
2. **Stat cards (fila superior)** — 4 cards horizontales:
   - Gateways Activos (contador, ícono verde)
   - Cámaras Online (contador, ícono teal)
   - Cámaras Offline (contador, ícono rojo)
   - Almacenamiento Cloud (%, barra de progreso azul)
3. **Grid de gateways** — cards por cada cliente/gateway con:
   - Nombre del gateway, estado badge (online/offline), IP, última conexión
   - Mini lista de cámaras vinculadas con sus estados
4. **Actividad reciente** — tabla o lista de últimos eventos MQTT (cámara online/offline, etc.)
5. **Almacenamiento** — barra de uso Cloud NAS vs NVR Local

**API que ya existe**:
- `GET /api/clients` → lista de clientes/gateways con `{ id, gatewayId, name, isActive, cameras: [...] }`
- `GET /health` → `{ status, database, uptime }`

**Clases CSS del design system** (ver `AI_RULES.md`):
- `.topbar`, `.card`, `.badge.online`, `.badge.offline`, `.badge.active`

---

### WENDY-5 — Cámaras (`/cameras`) — Node ID Pencil: `ZFh4R`

**Estado actual**: Solo se aplicó el fix de URL. El diseño visual no corresponde al Pencil.

**Implementar** en `cameras.component.html` + `cameras.component.scss`:

1. **Topbar** — título "Cámaras" + badge con total online + botón "Añadir Cámara"
2. **Filtros** — pills: Todas / Online / Offline / Cloud / NVR / SD
3. **Grid 2×2 (responsive)** — una card por cámara con:
   - Thumbnail / placeholder con ícono según estado (online=verde, offline=rojo)
   - Badge estado: "EN VIVO" (verde) o "OFFLINE" (rojo)
   - Badge tipo: "Cloud NAS" (azul) / "NVR" (teal) / "SD Card" (ámbar)
   - Nombre de la cámara, nombre del gateway
   - Botones:
     - Si online → "Ver en vivo" (abre player HLS: `GET /api/admin/stream/{id}/hls`)
     - Siempre → "Ver Grabaciones" (navega a `/recordings/{id}`)
   - Card offline: borde izquierdo rojo, ícono wifi-off, texto "Sin señal"
   - Card online: borde izquierdo teal

**API que ya existe**:
- `GET /api/cameras` → lista de cámaras con `{ id, name, clientId, gatewayId, streamType, isActive, ... }`
- `GET /api/admin/stream/{id}/hls` → URL del stream HLS (para el player)

---

### WENDY-6 — Grabaciones (`/recordings/:id`) — Node ID Pencil: `lAzQe`

**Estado actual**: Solo tiene dark mode CSS parcial. El layout completo no está implementado.

**Implementar** en `recordings.component.html` + `recordings.component.scss`:

1. **Topbar** — "Grabaciones · [Nombre Cámara]" + badge "X grabaciones hoy" + botón "Exportar CSV"
2. **Stat cards (4)**:
   - Total hoy (número)
   - Cloud NAS (%, barra azul)
   - NVR Local (%, barra teal)
   - Procesando (número, ámbar si > 0)
3. **Filtros** — búsqueda por texto + pills de tipo: Todas / Cloud / NVR / SD
4. **Tabla de grabaciones** con columnas: Cámara | Fecha/Hora | Duración | Tamaño | Tipo | Estado | Acciones
   - Estado badges: "Lista" (verde) / "Procesando" (ámbar) / "Sin acceso" (gris)
   - Acción "Descargar" solo cuando estado = Lista
   - Filas zebra (fila par ligeramente gris)

**API que ya existe**:
- `GET /api/recordings/cloud/{cameraId}?date=YYYY-MM-DD` → `{ date, cameraId, files: [{filename, path, sizeMb, startTime}] }`
- La respuesta tiene `files` como array anidado — acceder con `response.files`

---

## ✅ WENDY — Implementado (2026-02-23)

### WENDY-4 — Dashboard (`/dashboard`) — COMPLETO
- 4 stat cards: Gateways Activos / Cámaras Online / Cámaras Offline / Cloud NAS
- Gateways grid con borde indicador de color online/offline, mini-lista de cámaras vinculadas, cam-chips clickables que abren modal de stream
- Tabla de actividad reciente derivada de cameras + gateways
- Eliminados `app-motor-control` y `app-telemetry-dashboard` (violaban AI_RULES.md)
- `fetchHealth()` usa `GET /health` publicado por Bendi

### WENDY-5 — Cámaras (`/cameras`) — COMPLETO
- Filtros pill: Todas / Online / Offline + búsqueda por nombre/ubicación
- Grid de tarjetas premium: thumbnail NVR con scan-line effect, badges "EN VIVO" / "SIN SEÑAL" / Cloud NAS / NVR / SD Card
- Acciones por tarjeta: "Ver en Vivo" (solo si online) + "Ver Grabaciones" + editar/eliminar
- Stream corregido a `/api/admin/stream/{id}/hls` (WENDY-1)
- Modal de formulario: create/edit con dropdown de gateway

### WENDY-6 — Grabaciones (`/recordings/:id`) — COMPLETO
- 4 stat cards: Total Hoy / Cloud NAS MB / NVR Local / Procesando
- Filtros: date picker + búsqueda por nombre + pills de tipo
- Quick date pills para fechas con grabaciones disponibles
- Tabla: columnas Archivo / Fecha·Hora / Tamaño / Tipo / Estado / Acciones
  - Badges: "Lista" (verde) / Estado por defecto
  - Botones: "▶ Reproducir" + "⬇ Descargar"
- Video player integrado con cierre en el mismo componente
- Usa `response.files` correctamente (WENDY-2)
